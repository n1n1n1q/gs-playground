...
def save_image(view: CameraView, dir: str, img_name: str, save_new_images: bool = True):
...
def write_cameras_txt(cameras: List[Camera], dir: str):
    """
...
def write_images_txt(views: List[CameraView], dir: str, 
...
def write_points3D_txt(pcd: o3d.geometry.PointCloud, dir: str):
...
def write_cameras_binary(cameras, dir):
    """
...
def write_images_binary(views: List[CameraView], dir: str, conf_threshold: float = 0.0):
    """
    see: src/colmap/scene/reconstruction.cc
    ...
    with open(f"{dir}/images.bin", "wb") as fid:
        write_next_bytes(fid, len([v for v in views if v.confidence >= conf_threshold]), "Q")
        for id, view in enumerate(views):
            if view.confidence < conf_threshold:
                print(f"[WARNING] View {id + 1} has confidence {view.confidence}, which is below the threshold {conf_threshold}. Skipping this view.")
                continue
...
def write_points3D_binary(pcd, dir):
    """
    see: src/colmap/scene/reconstruction.cc
