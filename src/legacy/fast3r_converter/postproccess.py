...
def inference_to_pcds(preds, views, conf_threshold=0.0, debug=False):
    """
...
def downsample_per_frame(pointclouds, voxel_size=0.01):
    ...
    for i, pc in pointclouds.items():
        ...
        downsampled_clouds[i].points = o3d.utility.Vector3dVector(
            np.asarray(downsampled_pc.points)
        )
        downsampled_clouds[i].colors = o3d.utility.Vector3dVector(
            np.asarray(downsampled_pc.colors)
        )
